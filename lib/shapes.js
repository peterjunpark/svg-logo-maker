class Shape {
  render() {
    const innerSVG = this.shape + this.text;
    const svg = `<svg version="1.1" width="300" height="200">${innerSVG}</svg>`;
    console.log(svg);
    return svg;
  }
}
...
class Triangle extends Shape {
  constructor(text, textColor, bgColor) {
    super();
    this.shape = `<polygon points="50,200 250,200 150,0" style="${bgColor}" />`;
...
class Square extends Shape {
  constructor(text, textColor, bgColor) {
    ...
    this.text = `<text x="150" y="125" font-size="80" text-anchor="middle" fill="${textColor}">${text}</text></svg>`;
  }
}
